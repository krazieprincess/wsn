function [STATISTICS3,FD4,TD4,AD4]=EDGE(IniEng,NetSize,NoOfNode,NoOfRound,cluster_head_percentage)
...
for r=0:1:rmax     
    ...
    for i=1:1:n
        if(S(i).E>0)
            ...
            if ( (S(i).G)<=0)  
                if (temp_rand<= (p/(1-p*mod(r,round(1/p))))||distance<=do)
                    if (S(i).E>=Eavg)%%Improvement1
                        countCHs=countCHs+1;
                        ...
                        distance;
                        if (distance>do)
                            S(i).E=S(i).E- 2*( (ETX+EDA)*(4000)  + Efs*4000*( distance * distance )); %S(i).E=S(i).E- ( (ETX+EDA)*(4000) + Emp*4000*( distance*distance*distance*distance )); 
