function [STATISTICS2,FD2,TD2,AD2]=Improved1_leach(IniEng,NetSize,NoOfNode,NoOfRound,cluster_head_percentage)
...
for r=0:1:rmax     
    ...
    for i=1:1:n
        if(S(i).E>0)
            temp_rand=rand;     
            if ( (S(i).G)<=0)  
                if(temp_rand<= (p/(1-p*mod(r,round(1/p))))*((Eo-S(i).E)/Eo))
                    countCHs=countCHs+1;
